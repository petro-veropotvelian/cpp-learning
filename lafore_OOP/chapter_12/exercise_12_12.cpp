...
// Ask yourself whether the function should be static, virtual, or something else. This
...
public:
   ...
   static void find();                                               // find an employee by specified number
};
//-----------------------------------------------------------------------------
// static variables
...
void employee::write()
{
   ...
   ouf.open("EMPLOY_12.DAT", ios::trunc | ios::binary);
   if (!ouf)
   {
      cout << "\nCan't open file\n";
      return;
...
void employee::read()
{
   ...
   inf.open("EMPLOY_12.DAT", ios::binary);
   if (!inf)
   {
      cout << "\nCan't open file\n";
      return;
...
//-------------------------------------------------------------------
// find an employee by specified number
void employee::find()
{
   int nIndex;
   cout << "Please, specify index of employee: ";
   cin >> nIndex;
   if (nIndex <=0 )
   {
   ...
   int size;                                                         // size of employee object
   employee_type etype;                                              // type of employee
   employee* pEmployee;                                              // pointer to the employee object
   ifstream inf;                                                     // open ifstream in binary
   inf.open("EMPLOY_12.DAT", ios::binary);
   if (!inf)
   {
   ...
   while (true)
   {
      ...
      {                                                              // of a correct type
         case tmanager:
            ...
            break;
         case tscientist:
            pEmployee = new scientist;
            size = sizeof(scientist);
            break;
         case tlaborer:
            pEmployee = new laborer;
            size = sizeof(laborer);
            break;
         default:
            cout << "\nUnknown type in file\n";
      ...
      inf.read((char*)pEmployee, size);                               // read data from file into it
      if (!inf)
      {
         cout << "\nCan't read data from file\n";                    // error but not eof
         delete pEmployee;
      ...
      {
         cout << "\n   Type: ";
         if (pEmployee->get_type() == tmanager)
         {
            cout << "Manager";
         }
         else if (pEmployee->get_type() == tscientist)
         {
             cout << "Scientist";
         }
         else if (pEmployee->get_type() == tlaborer)
         {
            cout << "Laborer";
         }
         pEmployee->putdata();
   ...
   while (true)
   {
      ...
      switch (ch)
      {
         ...
         case 'r':
            ...
            break;
         case 'f':
            employee::find();                                        // find a specific employee by index
            break;
         case 'x':
            exit(0);                                                 // exit program
         default:
