// exercise_6_2.cpp
// Imagine a tollbooth at a bridge. Cars passing by the booth are expected to
// pay a 50 cent toll. Mostly they do, but sometimes a car goes by without
// paying. The tollbooth keeps track of the number of cars that have gone by,
// and of the total amount of money collected.
// Model this tollbooth with a class called tollBooth. The two data items are a
// type unsigned int to hold the total number of cars, and a type double to
// hold the total amount of money collected. A constructor initializes both of
// these to 0. A member function called payingCar() increments the car total
// and adds 0.50 to the cash total. Another function, called nopayCar(),
// increments the car total but adds nothing to the cash total. Finally, a
// member function called display() displays the two totals. Make appropriate
// member functions const.
// Include a program to test this class. This program should allow the user to
// push one key to count a paying car, and another to count a nonpaying car.
// Pushing the Esc key should cause the program to print out the total cars and
// total cash and then exit.
#include <iostream>
#include <windows.h>
#include <conio.h>
using namespace std;
/////////////////////////////////////////////////////////////////////////////////////////
#define ESCAPE 27
class tollBooth
{
private:
   unsigned int nCars;
   double dAmount;
public:
   tollBooth() : nCars(0), dAmount(0.0)
   {}
   void display() const;
   void payingCar();
   void nopayCar();
};
//---------------------------------------------------------------------------------------
void tollBooth::display() const
{
   cout << "Number of passing cars: " << nCars;
   cout << "\nAmount of money raised: $" << dAmount << endl;
}
//---------------------------------------------------------------------------------------
void tollBooth::payingCar()
{
   nCars++;
   dAmount+=0.50;
}
//---------------------------------------------------------------------------------------
void tollBooth::nopayCar()
{
   nCars++;
}
/////////////////////////////////////////////////////////////////////////////////////////
int main(int argc, char* argv[])
{
   cout << "Welcome to the tollbooth emulator!" << endl;
   char cResp;
   bool bContinue = true;
   tollBooth tb;
   while (bContinue)
   {
      cout << "Press \'P\' to simulate the passage of car with paying a toll,"
           << "\npress \'F\' to simulate the passage of the car without paying a toll."
           << "\nPress \'Esc\' to exit.";
       cResp = getch();
       switch (cResp)
       {
          case 'p':
          case 'P':
            tb.payingCar();
            system("CLS");
            break;
          case 'f':
          case 'F':
            tb.nopayCar();
            system("CLS");
            break;
          case ESCAPE:
            system("CLS");
            tb.display();
            bContinue = false;
            break;
          default:
            cout << "\nYou entered an invalid character!\n";
            cout << "Press <Enter> to continue...";
            cin.get();
            system("CLS");
            break;
       }
   }
   cout << "Bye!" << endl;
   return 0;
}