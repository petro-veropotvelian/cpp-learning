...
void init_graphics()
{
   ...
   LPCWSTR str1 = TEXT("CONOUT$");
   hConsole = CreateFile(str1, GENERIC_WRITE | GENERIC_READ,
      FILE_SHARE_READ | FILE_SHARE_WRITE,
      0L, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0L);
   // set to 80x25 screen size
   SetConsoleScreenBufferSize(hConsole, console_size);
   // set text to write on black
   SetConsoleTextAttribute(hConsole, (WORD)((0 << 4) | 15));
   fill_char = '\xDB';                                      // default fill is solib block
   clear_screen();
}
//---------------------------------------------------------------------------------------
...
void draw_rectangle(int left, int top, int right, int bottom)
{
   ...
   int j;
   for (j = 0; j < width; j++)                  // string of squared
   {
      temp[j] = fill_char;
   }
   temp[j] = 0;                                   // null
...
void draw_circle(int xC, int yC, int radius)
{
   ...
   {
      xF = radius * cos(theta);
      xN = static_cast<int>(xF * 2 / 1);                 // pixels not squared
      ...
      {                                                  // one for each half circle
         ...
         _putch(fill_char);                               // top
         set_cursor_pos(x++, yC + yN);
         _putch(fill_char);                               // bottom
      }
   }// end for
}
//---------------------------------------------------------------------------------------
