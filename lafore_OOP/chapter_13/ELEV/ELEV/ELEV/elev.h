...
#include <conio.h>                                                             // for screen ouptut
...
private:
   ...
   const int car_number;                                                       // out number (0 to nc-1)
   int current_floor;                                                          // where are we? (0 to nf-1)
   int old_floor;                                                              // where were we? (0 to nf-1)
   direction current_direction;                                                // which way are we going?
...
private:
   ...
   bool floor_request[2][NUM_FLOORS];                                          // array of up/down buttons
                                                                               // false = UP, true = DN
public:
   ...
   bool get_floor_req(const int, const int) const;                             // get specific floor req
